...
- 1. 【Impulse｜Unconditional｜Redstone Controlled｜Delay 0】
```command
...
```
- 2. 【Chain｜Unconditional｜Always Active｜Delay 0】
```command
execute as @p run clear @s[hasitem={item=emerald,quantity=64..}] emerald 0 64
```
- 3. 【Chain｜Conditional｜Always Active｜Delay 0】
```command
playsound random.orb @p
```
- 4. 【Chain｜Conditional｜Always Active｜Delay 0】
```command
scoreboard players add @p "Coins" 0
```
- 5. 【Chain｜Conditional｜Always Active｜Delay 0】
```commabd
give @p apple 64 0
```
</details>
...
<details>
<summary>Full Format Combination Table</summary>
